...
export default function Claim() {
  ...
  useEffect(() => {
    document.title = "Claim Listing | The Food Advisor";
    ...
    meta.setAttribute("content", "Claim your restaurant listing on The Food Advisor.");
    ...
    // ⭐ Replace Replit workspace API with real fetch call
    fetch(`https://thefoodadvisor.co.uk/api/claim`, {
      method: "POST",
      ...
      .catch((err: any) => {
        ...
        if (msg.toLowerCase().includes("invalid") || msg.toLowerCase().includes("token")) {
          setErrorMsg("This claim link is invalid or has expired.");
        } else {
          setErrorMsg(msg);
        }
      });
        ...
        <Card className="shadow-xl border-card-border/60 bg-card/80 backdrop-blur-xl relative overflow-hidden">
          ...
          <CardHeader className="text-center pb-4 pt-10 px-8">
            ...
            <CardTitle className="font-serif text-3xl text-foreground">
              ...
              {status === "success" && "Listing Successfully Claimed"}
          ...
          <CardContent className="text-center pb-10 px-8 pt-2">
            {status === "loading" && (
              <p className="text-muted-foreground text-lg animate-pulse">
                Please wait while we verify your claim token...
              </p>
            )}
            ...
            {status === "success" && (
              <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <p className="text-foreground/90 text-lg leading-relaxed">
                  Your restaurant listing has been successfully claimed. You now have full control over your venue profile.
                </p>

                <div className="bg-secondary/30 p-4 rounded-lg border border-border/50">
                  <p className="text-sm text-muted-foreground font-medium">
                    You can now update your menu, photos, contact details, and more.
                  </p>
                </div>
              </div>
            )}
